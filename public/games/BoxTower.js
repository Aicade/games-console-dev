...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // High Score Display
    this.highScore = localStorage.getItem('boxTowerHighScore') ? parseInt(localStorage.getItem('boxTowerHighScore')) : 0;
    ...
    this.matter.world.on('collisionstart', (event) => {
      event.pairs.forEach((pair) => {
        ...
        if (this.blocks.length > 0) {
          ...
          if ((pair.bodyA.id === lastBlock.body.id || pair.bodyB.id === lastBlock.body.id) && !lastBlock.landed) {
        ...
        if (bodyA.id == this.ground.body.id || bodyB.id == this.ground.body.id) {
          if (!this.groundCollided) {
            ...
            this.groundCollidedId = (bodyA.id == this.ground.body.id) ? bodyB.id : bodyA.id;
  ...
  addMovingObject() {
    ...
    this.movingBlock = this.matter.add.image(100, 100, 'player', null, { isStatic: true })
  ...
  handlePointerDown(pointer) {
    if (!this.onceGameOverCall && this.canDrop) {
      ...
      this.block = this.matter.add.image(x, y, 'player', null, { isStatic: false })
